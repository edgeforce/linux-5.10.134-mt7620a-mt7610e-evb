...
static irqreturn_t tdm_err_isr(int irq, void *p)
{
	...
	/* transmit errors */
	status = ioread32be(&priv->tdm_regs->ter);
	mask = ioread32be(&priv->tdm_regs->tier);
	val = status & mask;
	iowrite32be(val, &priv->tdm_regs->ter);
	...
	/* receive errors */
	status = ioread32be(&priv->tdm_regs->rer);
	mask = ioread32be(&priv->tdm_regs->rier);
	val = status & mask;
	iowrite32be(val, &priv->tdm_regs->rer);
	...
	dmac_err = ioread32be(&priv->dmac_regs->dmaes);
	...
	/* Clear the error */
	iowrite8((u8)ch, &priv->dmac_regs->dmacerr);
	return IRQ_HANDLED;
}
...
static irqreturn_t dmac_done_isr(int irq, void *p)
{
	...
	ch = ioread32be(&priv->dmac_regs->dmaintl);

	/* clear interrupt */
	if (ch & DMAC_RX_INT) {
		iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmacint);
		ret = IRQ_HANDLED;
	...
	if (ch & DMAC_TX_INT) {
		iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmacint);
		ret = IRQ_HANDLED;
	}
...
static int tdm_fsl_reg_init(struct tdm_priv *priv)
{
	...
	iowrite32be(DMACR_ERGA | DMACR_ERCA, &priv->dmac_regs->dmacr);
	/* Enable error Interrupts for TDM Rx &Tx */
	iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmaseei);
	iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmaseei);
	iowrite32be(DMAGPOR_SNOOP, &priv->dmac_regs->dmagpor);
	...
	if (adap->adapt_cfg.loopback)
		iowrite32be(GIR_LPBK | GIR_RTS, &priv->tdm_regs->gir);
	else
		iowrite32be(GIR_RTS, &priv->tdm_regs->gir);
	...
	iowrite32be(RIR_RFWM(RIR_RFWM_VAL) | RIR_RFEN | RIR_RWEN | RIR_RDMA |
		 RIR_RSL | RIR_RSO | RIR_RCOE | RIR_RRDO |
		 RIR_RFSD(RIR_RFSD_VAL),
		 &priv->tdm_regs->rir);
	iowrite32be(TIR_TFWM(TIR_RFWM_VAL) | TIR_TFEN | TIR_TWEN | TIR_TDMA |
	...
	iowrite32be(RFP_RNCF(adap->adapt_cfg.num_ch) | RFP_RCS(ch_size_type),
		 &priv->tdm_regs->rfp);
	iowrite32be(TFP_TNCF(adap->adapt_cfg.num_ch) | TFP_TCS(ch_size_type),
		 &priv->tdm_regs->tfp);

	iowrite32be(0, &priv->tdm_regs->rier);
	iowrite32be(0, &priv->tdm_regs->tier);

	/* clear all receive and transmit chs */
	for (i = 0; i < 4; i++) {
		iowrite32be(0, &priv->tdm_regs->tcma[i]);
		iowrite32be(0, &priv->tdm_regs->tcen[i]);
		iowrite32be(0, &priv->tdm_regs->rcen[i]);
	}
...
{
	/* stop the Tx & Rx */
	iowrite32be(0, &priv->tdm_regs->tcr);
	iowrite32be(0, &priv->tdm_regs->rcr);

	/* Clear DMA error Enable Request DMAEEIH/L */
	iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmaceei);
	iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmaceei);
	iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmacint);
	iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmacint);

	/* disable the dma request */
	iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmacerq);
	iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmacerq);
}
...
static int tdm_fsl_enable(struct tdm_adapter *adap)
{
	...
	for (i = 0; i < NUM_TDMTCEN_REG; i++) {
		iowrite32be(ch_enab[i], &priv->tdm_regs->rcen[i]);
		iowrite32be(ch_enab[i], &priv->tdm_regs->tcen[i]);
	}

	/* Clear the DONE bit */
	iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmacdne);
	iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmacdne);

	/* Load the Tx  transfer control descriptors */
	for (i = 0; i < DMA_MAX_TCD; i++)
		iowrite32be(priv->dma_tx_tcd[ph]->tcd[i],
			 &priv->dmac_regs->tcd[TDMTX_DMA_CH].tcd[i]);

	/* Load the Rx  transfer control descriptors */
	for (i = 0; i < DMA_MAX_TCD; i++)
		iowrite32be(priv->dma_rx_tcd[ph]->tcd[i],
			 &priv->dmac_regs->tcd[TDMRX_DMA_CH].tcd[i]);

	/* enable the dma request */
	iowrite8(TDMRX_DMA_CH, &priv->dmac_regs->dmaserq);
	iowrite8(TDMTX_DMA_CH, &priv->dmac_regs->dmaserq);

	/* Enable Receiver, transmitter */
	timeout = jiffies + TDM_ENABLE_TIMEOUT;
	iowrite32be(TCR_TEN, &priv->tdm_regs->tcr);
	while (!(ioread32be(&priv->tdm_regs->tsr) & TSR_TENS)) {
	...
	timeout = jiffies + TDM_ENABLE_TIMEOUT;
	iowrite32be(RCR_REN, &priv->tdm_regs->rcr);
	while (!(ioread32be(&priv->tdm_regs->rsr) & RSR_RENS)) {
		if (time_after(jiffies, timeout)) {
		...
		u16 **output_tdm_buffer)
{
	...
	tmp = ioread32be(&priv->dmac_regs->tcd[TDMTX_DMA_CH].tcd[0]);
